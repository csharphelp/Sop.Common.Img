...
{
    public class ImgAnimateGifTest
    {
        ...
        public void GetGifImage_Test()
        {
            ...
            var isok = ImgAnimateGif.Instance().GetGifImage(imageFilePaths, outputImagePath);
        ...
        /// </summary>
        [Test]
        public void SetGifImage_Test()
        {
            var isExists = File.Exists(imageGifPath);
            Assert.IsTrue(isExists, "文件存在");

            var success = ImgAnimateGif.Instance().SetGifImage(imageGifPath, outputFilePath, out var list);


            Assert.AreEqual(list.Count, 3);
            Assert.IsTrue(success, "成功");
        }

        #region Png_Img_To_Gif &&Gif_Img_To_Png
        /// <summary>
        /// 生成gif图片
        /// </summary>
        [Test]
